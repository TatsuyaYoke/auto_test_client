...
const readGroundDbSync = (queryObject: {
  path: string
  query: string
  notExistColumns: string[]
...
export const getGroundData = async (request: RequestDataType, dbTopPath: string | null): Promise<ResponseDataType> => {
  if (dbTopPath) {
    ...
    const columnCheckResults = await Promise.all(
      tableCheckResults.map(async (result) => {
        if (result.success) {
          ...
          const filteredNotExistColumns = uniqueArray(
            notExistColumns.filter(
              (notExistColumn) =>
                !existColumnsList
    ...
    const queryObjectList = queryTargetList.map((queryTarget) => {
      ...
      const queryJoin = trimQuery(
        queryTarget.existColumnsList
          .slice(1, queryTarget.existColumnsList.length)
          .reduce((prevQuery, currentElement) => {
